...
### 附录 §2.1.2-a：vault 与 mindmap 的副本语义分化（2026-08-29 · P0 修复记录）

vault 与 mindmap 的副本更新语义**有意分化**，非疏漏：

| 类别 | 副本语义 | 理由 |
|---|---|---|
| mindmap（.local.json） | 首次冲突写入，永不覆盖 | 布局/结构可从笔记重建，历史分叉点有考古价值 |
| vault（.md.conflict） | **每次冲突更新**为最近一次被覆盖的本地版本 | vault 是 Layer 1 唯一知识真相源（AGENTS §3 数据永不锁死）；用户可能基于上一版做新编辑，若副本停在首份分叉，新编辑会永久丢失（2026-08-29 实证 P0） |

代价：更早的历史版本不被保留（上一轮同步后已暴露给用户）。备选的序号追加
（零丢失但文件堆积）与 fail-closed（不丢但阻塞同步）均劣于本方案。
