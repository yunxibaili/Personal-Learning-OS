...
describe("Focus (S4) — 只留 Work Surface", () => {
  it("Focus 下 Explorer/Context 均让位（none）", () => {
    const p = computeLayout({ explorer: true, context: true, compare: false, focus: true, inspector: false, width: 1440 });
...
describe("Compare (S3) — 连续双工作场", () => {
  it("Compare 下均分两列，Context 让位（S3 隐藏 Context 以保持 quiet）", () => {
    const p = computeLayout({ explorer: false, context: true, compare: true, focus: false, inspector: false, width: 1920 });
...
describe("Context-only — P0 修复（不得再塌陷为 264px）", () => {
  it("1440 仅开 Context：surface 为 1fr，context 为独立列", () => {
    const p = computeLayout({ explorer: false, context: true, compare: false, focus: false, inspector: false, width: 1440 });
  ...
  it("2560 仅开 Context：Work Surface 仍为主列", () => {
    const p = computeLayout({ explorer: false, context: true, compare: false, focus: false, inspector: false, width: 2560 });
...
describe("Explorer + Context 同时开启 — 让位规则", () => {
  it("1280 宽度不足 → Context 让位为抽屉，Work Surface 不被压到 680 以下", () => {
    const p = computeLayout({ explorer: true, context: true, compare: false, focus: false, inspector: false, width: 1280 });
  ...
  it("1920 空间充足 → 两者皆可成列", () => {
    const p = computeLayout({ explorer: true, context: true, compare: false, focus: false, inspector: false, width: 1920 });
    expect(p.explorerMode).toBe("column");
    expect(p.contextMode).toBe("column");
...
describe("Reading measure 策略", () => {
  it("≥1920 放宽至 760；<1920 目标 680", () => {
    expect(computeLayout({ explorer: false, context: false, compare: false, focus: false, inspector: false, width: 1920 }).readerMeasure).toBe(760);
    expect(computeLayout({ explorer: false, context: false, compare: false, focus: false, inspector: false, width: 1440 }).readerMeasure).toBe(680);
  });

  it("768 窄屏：仍保持 Work Surface 单列（抽屉化由 CSS 负责）", () => {
    const p = computeLayout({ explorer: false, context: true, compare: false, focus: false, inspector: false, width: 768 });
