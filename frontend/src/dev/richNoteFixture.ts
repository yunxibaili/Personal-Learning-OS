...
\`\`\`ts
function gradientStep(theta: number[], grad: number[], lr: number): number[] {
  // 沿负梯度方向更新
  return theta.map((t, i) => t - lr * grad[i]);
}
\`\`\`
...
判断优化是否健康，工程上有三个常用信号：损失曲线是否平滑下降、梯度范数是否稳定、参数更新幅度是否与学习率匹配。损失突然变为 \`NaN\` 通常意味着学习率过大或出现了数值溢出；此时应优先检查数据归一化与梯度裁剪。

在\`Transformer\`这类深层架构中，通常配合 [[学习率调度]] 使用 warmup：前若干步用极小的学习率热身，再按余弦或线性策略衰减。这些策略的参数同样通过梯度下降的整体训练动态来观察与调整。
