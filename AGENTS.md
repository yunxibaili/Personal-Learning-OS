...
> **⚠️ 当前开发政策：后端稳定阶段（Backend-only Stable Phase，2026-09-05 裁定，现行有效）** ——
> 见 `docs/PROJECT_STATE.md` §0（唯一权威）。要点：前端代码当前不存在（`web/` `ui/` `shared/`
> 已移除），旧前端阶段（P1 UI 打磨 / P2 完整验收 / Bright Baseline）**已终止而非延期**；
> ADR-028 后端核心**已封口**；下一阶段由 Owner 明确解冻，**不得隐式解释为前端**。
>
> 历史政策（仅存档，不再约束）：后端优先（2026-08-28）· 前端阶段（2026-08-30）·
> P8 收尾阶段（2026-08-31，其「端到端闭环 + 契约一致性 + 跨层修改须真实原因」准则仍作为
> 工程纪律有效，见 §12）。
